...
import mollie.MolliePaymentModel;
...
//        return getCartPageRedirect();
     ...
     * @return ModelAndView
     */
    public String paypalProcess(HttpServletRequest request, HttpServletResponse response
    		, 
    		Model model
    		,
                                      @RequestParam 
...
//    		,@RequestParam("id") String paymentId
        ...
        Order cart = CartState.getCart(); //ova moze da ostane
        if (!(cart instanceof NullOrderImpl)) {
...
//    	System.out.println("vojdan");
        return getCartPageRedirect();
    }
    
    public String paypalProcessMIRZ(HttpServletRequest request, HttpServletResponse response, @RequestParam("id")  String paymentId ) throws CheckoutException, PricingException, ClientProtocolException, IOException {
...
//                orderService.save(cart, false);
//
//                CheckoutResponse checkoutResponse = payPalCheckoutService.completeExpressCheckout(paymentId, cart);
//                PaymentResponseItem responseItem = checkoutResponse.getPaymentResponse().getResponseItems().get(payPalPaymentInfo);
//                if (!responseItem.getTransactionSuccess()) {
//                    populateModelWithShippingReferenceData(request, model);
//                    model.addAttribute("paymentException", true);
...
//                Order order  = orderService.save(checkoutResponse.getOrder(), false);
//                seed = new CheckoutSeed(order, null, new HashMap<String, Object>());
//
...
//                seed.getOrder().setSubmitDate(Calendar.getInstance().getTime());
                ...
                return getConfirmationView("123");
...
//    	System.out.println("vojdan");
    ...
    public String mirDoGet(Order cart) {
		...
		try {
			...
			StringEntity params = new StringEntity(
					"{\"amount\":\"" +
					cart.getTotal() + //"101.00" +
					"\",\"redirectUrl\":\"" +
					"http://88.85.115.54/paypal/process" +
					"\",\"description\":\"" +
					//cart.getOrderNumber() +
					"Pharmaskincare.nl" + 
					//"Order #40029" +
...
//			}
			...
			return paymentInfo.getLinks().getPaymentUrl();
			//return;
    ...
    public String mirDoPost(String paymentId) throws ClientProtocolException, IOException{
		HttpClient httpClient = new DefaultHttpClient();
		HttpGet requestV = new HttpGet("https://api.mollie.nl/v1/payments/"+paymentId);
		...
		return getConfirmationView(paymentInfo.getDescription());
